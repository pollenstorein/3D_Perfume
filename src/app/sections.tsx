...
import { default as giftGalleryTwo, default as giftSetDetailOne } from "./Images/4b.PNG";
...
export function MomentSection() {
  return <section className="relative h-[clamp(520px,78vh,860px)] overflow-hidden bg-black md:aspect-[3375/4219] md:h-auto"><img src={momentImage} alt="Find yourself in the moment" className="absolute inset-0 h-full w-full scale-[0.94] object-cover md:scale-100 md:object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/65 via-black/10 to-transparent" /><div className="absolute inset-x-6 bottom-12 z-10 text-center text-white md:bottom-16"><h2 className="text-2xl font-medium uppercase tracking-[0.08em] md:text-4xl">"Find yourself in the moment"</h2><button type="button" className="mt-6 border border-white bg-transparent px-6 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-white transition-colors hover:bg-white hover:text-black">Know Pollen</button></div></section>;
}
...
export function PricingSection({ onAddToCart, cartItems, onChangeQuantity }: { onAddToCart: (fragrance: typeof FRAGRANCES[number]) => void; cartItems: { id: number; quantity: number }[]; onChangeQuantity: (id: number, change: number) => void }) {
  return <section className="bg-black px-4 py-14 text-white sm:px-6 md:px-10 md:py-20"><div className="mx-auto max-w-screen-2xl"><h2 className="mb-10 text-center text-2xl font-medium uppercase tracking-[0.08em] sm:text-3xl md:mb-14 md:text-4xl">Shop the collection</h2><div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-3 md:gap-5 lg:gap-8">{PRICING_CARDS.map(({ image, fragrance }) => { const quantity = cartItems.find(item => item.id === fragrance.id)?.quantity ?? 0; return <article key={fragrance.id} className="mx-auto flex w-full max-w-sm flex-col text-center"><div className="aspect-[4/5] overflow-hidden bg-white"><img src={image} alt={fragrance.name} className="h-full w-full object-cover transition-transform duration-500 hover:scale-105" /></div><div className="flex flex-1 flex-col items-center px-2 pt-5"><h3 className="text-sm font-semibold uppercase tracking-[0.08em] sm:text-base">{fragrance.name}</h3><p className="mt-3 text-sm text-white/80">₹{fragrance.price.toLocaleString()}</p>{quantity === 0 ? <button type="button" onClick={() => onAddToCart(fragrance)} className="mt-6 bg-white px-5 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-black transition-colors hover:bg-neutral-200">Add to cart</button> : <div className="mt-6 inline-flex items-center border border-white text-sm font-semibold"><button type="button" onClick={() => onChangeQuantity(fragrance.id, -1)} aria-label={`Decrease ${fragrance.name} quantity`} className="flex h-10 w-10 items-center justify-center transition-colors hover:bg-white hover:text-black">-</button><span className="flex h-10 min-w-10 items-center justify-center px-2" aria-live="polite">{quantity}</span><button type="button" onClick={() => onAddToCart(fragrance)} aria-label={`Increase ${fragrance.name} quantity`} className="flex h-10 w-10 items-center justify-center transition-colors hover:bg-white hover:text-black">+</button></div>}</div></article>; })}</div></div></section>;
}
...
export function GiftSetSection({ onAddBundle, onBack }: { onAddBundle: () => void; onBack: () => void }) {
  return <section className="min-h-screen bg-white px-1 pb-16 pt-8 sm:px-4 md:px-8 md:pt-12"><div className="mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)] lg:gap-16"><div className="grid grid-cols-2 gap-1 sm:gap-2"><div className="col-span-2 aspect-[1.4/1] overflow-hidden bg-[#f3f0ed]"><img src={giftSetHero} alt="The Legacy Set gift box" className="h-full w-full object-cover" /></div><div className="aspect-[0.86/1] overflow-hidden bg-[#f3f0ed]"><img src={giftSetDetailOne} alt="The Legacy Set held in hand" className="h-full w-full object-cover" /></div><div className="aspect-[0.86/1] overflow-hidden bg-[#f3f0ed]"><img src={giftSetDetailTwo} alt="The Legacy Set lifestyle detail" className="h-full w-full object-cover" /></div></div><div className="flex flex-col justify-center px-5 py-4 lg:px-0"><button type="button" onClick={onBack} className="mb-12 inline-flex w-fit items-center text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50 hover:text-black">← Back to collection</button><p className="text-[10px] font-semibold uppercase tracking-[0.18em]">Every day · Every mood · Every side of you</p><h1 className="mt-3 text-3xl font-bold uppercase tracking-[0.08em] sm:text-4xl">The Legacy Set</h1><p className="mt-2 text-lg font-semibold uppercase tracking-[0.08em]">(4 × 20ML)</p><div className="mt-5 flex flex-wrap gap-2 text-[10px] uppercase"><span className="bg-black/10 px-3 py-1">Parfum</span><span className="bg-black/10 px-3 py-1">Unisex</span><span className="bg-black/10 px-3 py-1">Gift set</span></div><p className="mt-5 max-w-md text-sm leading-relaxed text-black/65">Four signatures. One legacy. The whole of Sarkar, in one box.</p><div className="mt-6 flex items-center gap-5"><span className="text-2xl">₹1,899</span><span className="text-lg text-black/45 line-through">₹1,999</span><span className="bg-black px-4 py-2 text-[10px] font-bold uppercase text-white">Save 5%</span></div><p className="mt-3 text-xs text-black/60">Incl. of all taxes</p><button type="button" onClick={onAddBundle} className="mt-5 w-full bg-black px-5 py-4 text-xs font-bold uppercase tracking-[0.12em] text-white transition-colors hover:bg-neutral-800">Add to cart</button><p className="mt-8 text-xs text-black/65">* Ships within 24-36 hours of ordering.</p><div className="mt-12 border-t border-black/10 pt-5"><p className="text-sm font-semibold">Offers</p><div className="mt-3 flex min-h-32 border border-black/10"><div className="flex w-10 items-center justify-center bg-black text-[9px] font-semibold uppercase tracking-[0.12em] text-white [writing-mode:vertical-rl]">Gift · included</div><div className="flex flex-1 flex-col justify-center px-4"><p className="text-lg font-semibold leading-tight">A mini surprise for you</p><p className="mt-4 text-xs text-black/55">Get a 7ml Parfum<br />with your order</p></div><div className="w-24 overflow-hidden bg-[#f3f0ed] sm:w-28"><img src={giftSetDetailTwo} alt="Mini parfum gift" className="h-full w-full object-cover" /></div></div></div></div></div></section>;
}
...
export function GiftSetGallerySection({ onAddBundle, onBack }: { onAddBundle: () => void; onBack: () => void }) {
  const [activeImage, setActiveImage] = useState(0);
  const touchStartX = useRef<number | null>(null);
  const changeImage = (direction: number) => setActiveImage(index => (index + direction + GIFT_SET_GALLERY.length) % GIFT_SET_GALLERY.length);

  return <section className="min-h-screen bg-white px-1 pb-16 pt-8 sm:px-4 md:px-8 md:pt-12"><div className="mx-auto grid max-w-[1440px] gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)] lg:gap-16"><div className="min-w-0"><div className="relative overflow-hidden bg-[#f3f0ed] md:hidden" onTouchStart={event => { touchStartX.current = event.touches[0].clientX; }} onTouchEnd={event => { if (touchStartX.current === null) return; const distance = touchStartX.current - event.changedTouches[0].clientX; touchStartX.current = null; if (Math.abs(distance) > 40) changeImage(distance > 0 ? 1 : -1); }}><img src={GIFT_SET_GALLERY[activeImage].image} alt={GIFT_SET_GALLERY[activeImage].alt} className="aspect-[0.86/1] h-full w-full object-cover" /><button type="button" onClick={() => changeImage(-1)} aria-label="Previous gift set image" className="absolute left-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center bg-white/85"><ArrowLeft size={16} /></button><button type="button" onClick={() => changeImage(1)} aria-label="Next gift set image" className="absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center bg-white/85"><ArrowRight size={16} /></button><div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5">{GIFT_SET_GALLERY.map((item, index) => <button key={item.alt} type="button" onClick={() => setActiveImage(index)} aria-label={`Show gift set image ${index + 1}`} className={`h-1.5 w-1.5 rounded-full ${index === activeImage ? "bg-black" : "bg-white"}`} />)}</div></div><div className="hidden grid-cols-4 gap-1 sm:gap-2 md:grid"><div className="col-span-4 aspect-[1.4/1] overflow-hidden bg-[#f3f0ed]"><img src={GIFT_SET_GALLERY[0].image} alt={GIFT_SET_GALLERY[0].alt} className="h-full w-full object-cover" /></div>{GIFT_SET_GALLERY.slice(1).map(item => <div key={item.alt} className="aspect-[0.86/1] overflow-hidden bg-[#f3f0ed]"><img src={item.image} alt={item.alt} className="h-full w-full object-cover" /></div>)}</div></div><div className="flex flex-col justify-center px-5 py-4 lg:px-0"><button type="button" onClick={onBack} className="mb-12 inline-flex w-fit items-center text-[10px] font-semibold uppercase tracking-[0.18em] text-black/50 hover:text-black">← Back to collection</button><p className="text-[10px] font-semibold uppercase tracking-[0.18em]">Every day · Every mood · Every side of you</p><h1 className="mt-3 text-3xl font-bold uppercase tracking-[0.08em] sm:text-4xl">The Legacy Set</h1><p className="mt-2 text-lg font-semibold uppercase tracking-[0.08em]">(4 × 20ML)</p><div className="mt-5 flex flex-wrap gap-2 text-[10px] uppercase"><span className="bg-black/10 px-3 py-1">Parfum</span><span className="bg-black/10 px-3 py-1">Unisex</span><span className="bg-black/10 px-3 py-1">Gift set</span></div><p className="mt-5 max-w-md text-sm leading-relaxed text-black/65">Four signatures. One legacy. The whole of Sarkar, in one box.</p><div className="mt-6 flex items-center gap-5"><span className="text-2xl">₹1,899</span><span className="text-lg text-black/45 line-through">₹1,999</span><span className="bg-black px-4 py-2 text-[10px] font-bold uppercase text-white">Save 5%</span></div><p className="mt-3 text-xs text-black/60">Incl. of all taxes</p><button type="button" onClick={onAddBundle} className="mt-5 w-full bg-black px-5 py-4 text-xs font-bold uppercase tracking-[0.12em] text-white transition-colors hover:bg-neutral-800">Add to cart</button><p className="mt-8 text-xs text-black/65">* Ships within 24-36 hours of ordering.</p><div className="mt-12 border-t border-black/10 pt-5"><p className="text-sm font-semibold">Offers</p><div className="mt-3 flex min-h-32 border border-black/10"><div className="flex w-10 items-center justify-center bg-black text-[9px] font-semibold uppercase tracking-[0.12em] text-white [writing-mode:vertical-rl]">Gift · included</div><div className="flex flex-1 flex-col justify-center px-4"><p className="text-lg font-semibold leading-tight">A mini surprise for you</p><p className="mt-4 text-xs text-black/55">Get a 7ml Parfum<br />with your order</p></div><div className="w-24 overflow-hidden bg-[#f3f0ed] sm:w-28"><img src={giftGalleryFive} alt="Mini parfum gift" className="h-full w-full object-cover" /></div></div></div></div></div></section>;
}

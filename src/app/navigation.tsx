...
export function TopBar({ menuOpen, onMenuToggle, user, onBuyNow, cartCount, onCartOpen }: { menuOpen: boolean; onMenuToggle: () => void; user: { name: string; email: string } | null; onBuyNow: () => void; cartCount: number; onCartOpen: () => void }) {
  ...
  return (
    <>
    <header className="fixed inset-x-0 top-0 z-30 px-4 sm:px-6 md:px-10 transition-all duration-300" style={{ height: scrolled ? "56px" : "72px", background: "rgba(255,255,255,0.68)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(0,0,0,0.14)" }}>
      <div className="mx-auto flex h-full w-full max-w-screen-xl items-center justify-between">
        <div className="flex items-center gap-3 sm:gap-4"><HamburgerButton onClick={onMenuToggle} open={menuOpen} /><button type="button" onClick={() => window.location.reload()} aria-label="Refresh Know Pollen" className="text-[10px] sm:text-xs md:text-sm font-bold tracking-[0.22em] sm:tracking-[0.28em] uppercase text-black select-none whitespace-nowrap">Know Pollen</button></div>
...
export function SideMenu({ open, onClose, onBuyNow, onOpenGiftSet, onOpenFragrance, onOpenCollection = () => {}, user, onLogin, onProfileSettings, onSignOut, onOpenOrderHistory }: { open: boolean; onClose: () => void; onBuyNow: () => void; onOpenGiftSet: () => void; onOpenFragrance: (id: number) => void; onOpenCollection?: () => void; user: { name: string; email: string } | null; onLogin: () => void; onProfileSettings: () => void; onSignOut: () => void; onOpenOrderHistory: () => void }) {
      ...
      {open && <>
        ...
        <motion.aside key="drawer" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ duration: 0.45, ease: [0.32, 0, 0.08, 1] }} className="fixed top-0 left-0 z-50 flex h-full w-full flex-col overflow-y-auto bg-white" style={{ width: "min(360px, 85vw)", borderRight: "1px solid #e0e0e0" }}>
          ...
          <nav className="flex-1 px-8 py-10 flex flex-col gap-1">
            ...
            {user && <button type="button" onClick={() => { onClose(); onOpenOrderHistory(); }} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-left text-black hover:border-black"><span className="text-sm font-semibold uppercase tracking-[0.12em]">Order history</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></button>}
            {NAV_ITEMS.filter(item => item.label === "GET YOUR BUNDLE").map(item => <motion.button key={item.label} type="button" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1, duration: 0.35 }} onClick={() => { onClose(); onOpenGiftSet(); }} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-left text-black transition-colors duration-200 hover:border-black"><span className="inline-block text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">Gift Set</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></motion.button>)}
            {NAV_ITEMS.filter(item => item.label === "SHOP").map(item => <motion.a key={item.label} href={item.href} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.155, duration: 0.35 }} onClick={onClose} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-black transition-colors duration-200 hover:border-black" style={{ textDecoration: "none" }}><span className="inline-block text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">Shop the collection</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></motion.a>)}
            <div className="border-b border-[#f0f0f0]">
              <button type="button" onClick={() => setFragrancesOpen(openState => !openState)} aria-expanded={fragrancesOpen} className="group flex w-full items-center justify-between py-4 text-left text-black transition-colors duration-200 hover:border-black"><span className="text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">Fragrances</span><ChevronDown size={15} className={`transition-transform duration-200 ${fragrancesOpen ? "rotate-180" : ""}`} /></button>
              <AnimatePresence initial={false}>{fragrancesOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pb-2">{FRAGRANCES.map(fragrance => <button key={fragrance.id} type="button" onClick={() => { onClose(); if (fragrance.id === 1) onOpenFragrance(fragrance.id); else window.location.hash = "fragrances"; }} className="block w-full py-2 pl-3 text-left text-xs font-medium uppercase tracking-[0.12em] text-black/60 hover:text-black">{fragrance.name}</button>)}</motion.div>}</AnimatePresence>
            </div>
            <button type="button" className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-left text-black transition-colors duration-200 hover:border-black"><span className="inline-block text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">Build your own</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></button>
            {NAV_ITEMS.filter(item => item.label === "KNOW POLLEN").map(item => <motion.a key={item.label} href={item.href} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.265, duration: 0.35 }} onClick={onClose} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-black transition-colors duration-200 hover:border-black" style={{ textDecoration: "none" }}><span className="inline-block text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">Know Pollen</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></motion.a>)}
            {!user && <button type="button" onClick={() => { onClose(); onLogin(); }} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-left text-black hover:border-black"><span className="text-sm font-semibold uppercase tracking-[0.12em]">Login</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></button>}
          </nav>

...
export function SideMenu({ open, onClose, onBuyNow, onOpenGiftSet, onOpenCollection = () => {}, user, onLogin, onProfileSettings, onSignOut, onOpenOrderHistory }: { open: boolean; onClose: () => void; onBuyNow: () => void; onOpenGiftSet: () => void; onOpenCollection?: () => void; user: { name: string; email: string } | null; onLogin: () => void; onProfileSettings: () => void; onSignOut: () => void; onOpenOrderHistory: () => void }) {
        ...
        <motion.aside key="drawer" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ duration: 0.45, ease: [0.32, 0, 0.08, 1] }} className="fixed top-0 left-0 z-50 flex h-full w-full flex-col overflow-y-auto bg-white" style={{ width: "min(360px, 85vw)", borderRight: "1px solid #e0e0e0" }}>
          ...
          <nav className="flex-1 px-8 py-10 flex flex-col gap-1">
            ...
            <div className="border-b border-[#f0f0f0]">
              ...
              <AnimatePresence initial={false}>{fragrancesOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pb-2">{FRAGRANCES.map(fragrance => <a key={fragrance.id} href="#fragrances" onClick={onClose} className="block py-2 pl-3 text-xs font-medium uppercase tracking-[0.12em] text-black/60 hover:text-black">{fragrance.name}</a>)}</motion.div>}</AnimatePresence>
            </div>

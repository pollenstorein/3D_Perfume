...
export function SideMenu({ open, onClose, onBuyNow, user, onLogin, onProfileSettings, onSignOut }: { open: boolean; onClose: () => void; onBuyNow: () => void; user: { name: string; email: string } | null; onLogin: () => void; onProfileSettings: () => void; onSignOut: () => void }) {
      ...
      {open && <>
        ...
        <motion.aside key="drawer" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ duration: 0.45, ease: [0.32, 0, 0.08, 1] }} className="fixed top-0 left-0 z-50 flex h-full w-full flex-col overflow-y-auto bg-white" style={{ width: "min(360px, 85vw)", borderRight: "1px solid #e0e0e0" }}>
          ...
          <nav className="flex-1 px-8 py-10 flex flex-col gap-1">
            ...
            {NAV_ITEMS.filter(item => item.label === "GET YOUR BUNDLE").map(item => <motion.a key={item.label} href={item.href} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1, duration: 0.35 }} onClick={onClose} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-black transition-colors duration-200 hover:border-black" style={{ textDecoration: "none" }}><span className="inline-block text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">Get the bundle</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></motion.a>)}
            ...
            {NAV_ITEMS.filter(item => item.label === "TRACK ORDER").map(item => <motion.a key={item.label} href={item.href} onClick={onClose} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-black transition-colors duration-200 hover:border-black" style={{ textDecoration: "none" }}><span className="inline-block text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">{item.label}</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></motion.a>)}
            {NAV_ITEMS.filter(item => item.label === "BUY NOW").map(item => <button key={item.label} type="button" onClick={() => { onClose(); onBuyNow(); }} className="group flex items-center justify-between border-b border-[#f0f0f0] py-4 text-left text-black transition-colors duration-200 hover:border-black"><span className="inline-block text-sm font-semibold uppercase tracking-[0.12em] transition-transform duration-200 group-hover:translate-x-1">{item.label}</span><ArrowRight size={13} className="opacity-0 transition-opacity group-hover:opacity-100" /></button>)}
          </nav>
